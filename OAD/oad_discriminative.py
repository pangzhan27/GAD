## Include instruction.
#  <|begin_of_text|>What is the action in the last frame?
#   <v> ... <v>
#   Cls_head with bias on an extra learnable embedding , no generation connector
# steps: 1. add cls_emb and add it to finetune list. 2. forward, append the class embedding to input and modify attention mask
...
def chat_template(self, stream_placeholder_jinja2: str, text2visual_connector: str, visual2text_connector: str):
    ...
    template = (
        ...
        "{{ V2T_PLACEHOLDER + message['content']}}"
...
class LiveLlamaForCausalLM(LlamaForCausalLM, LiveMixin):
    ...
    def forward(
        ...
        extra_embeds = self.cls_emb.weight.squeeze(0).repeat(inputs_embeds.shape[0], 1, 1)
        inputs_embeds = torch.cat([inputs_embeds, extra_embeds], dim=1)
        if attention_mask is not None:
            pad_att = torch.ones((attention_mask.shape[0], 1)).to(attention_mask.device).to(attention_mask.dtype)
            attention_mask = torch.cat([attention_mask, pad_att], dim=1)
    ...
    def _call_for_response(self, ):
        while self.frame_embeds_queue:
            ...
            inputs_embeds = torch.cat([
                ...
                frame_embeds.view(1, -1, self.hidden_size),], dim=1)

            outputs = self.model(inputs_embeds=inputs_embeds, past_key_values=None, use_cache=False)
            # decide
            next_score = outputs.logits[:, 0, :].softmax(dim=-1)
