### generation
# remove <cls> in template and learnable parameters
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.weight is not None:
                ...
                gen_loss = nn.functional.cross_entropy(gen_logits, labels, reduction='none')
                gen_tmpt_loss = gen_loss.sum(dim=-1) / ((labels >= 0).sum(dim=-1) + 1e-8)
                bg_loss = torch.sum(gen_tmpt_loss * bg_mask) / (torch.sum(bg_mask) + 1e-8)
                fg_loss = torch.sum(gen_tmpt_loss * fg_mask) / (torch.sum(fg_mask) + 1e-8)
                weight = bg_mask * self.config.stream_loss_weight + (1 - bg_mask)
                loss = torch.mean(gen_tmpt_loss * weight)
...
class LiveInfer_nocache:
    def __init__(self, model, tokenizer, args, classnames) -> None:
        ...
        self._sys_ids = self._sys_ids_tot[:,:-2]
        self._added_generation_ids_ass = self.tokenizer.apply_chat_template([{}], add_generation_prompt=True,  return_tensors='pt').to(self.device)
        self._added_generation_ids = torch.cat([self._sys_ids_tot[:, -2:], self._added_generation_ids_ass], dim=-1)
        self._eos_ids = torch.tensor([[self.eos_token_id]]).to(self._added_generation_ids.dtype).to(self.device)

        # for j in self._sys_ids[0, :]:
        #     print(j, repr(tokenizer.decode(j)))
        # print('##########################')
        # for j in self._added_generation_ids[0, :]:
        #     print(j, repr(tokenizer.decode(j)))
    ...
    def _call_for_response(self, video_time):
        ...
        output_ids, past_key_values = self.fast_greedy_generate(inputs_embeds=inputs_embeds,
                                                                past_key_values=self.past_key_values)

        out_label = self.tokenizer.decode(torch.cat([self.last_ids, output_ids], dim=-1)[0], skip_special_tokens=True,
