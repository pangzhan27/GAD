# generation the current action + task
# for task recognition, just ouput task
...
from transformers.models.llama.modeling_llama import LLAMA_INPUTS_DOCSTRING, KwargsForCausalLM, LlamaModel, \
    _CONFIG_FOR_DOC
...
def build_live_vision(config: LiveConfigMixin):
    ...
    if 'google/siglip-large-patch16-384' == config.vision_pretrained:
        return model, partial(_siglip_vision_encode, frame_token_cls=config.frame_token_cls,
        ...
        for r, (turn_start, turn_stop) in enumerate(zip(turn_starts, turn_stops)):
            ...
            if turn_stream_mask.any():
                ...
                if frame_token_interval_threshold > 0:
                    lower_threshold_mask = turn_stream_masked_score[:,
                                           frame_token_interval_id] < frame_token_interval_threshold
                ...
                if turn_stream_masked_pred_mask.any():
                    ...
                    past_key_values_before_assistant = self.trim_past_key_values(past_key_values, 0,
                    ...
                    if r == num_turns - 1:  # no future frame. we assume the model should receive a signal when streaming ends (e.g. close button).
                        ...
                        next_turn_num_frames = (input_id[turn_starts[r + 1]:turn_stops[
                            r + 1]] == v_placeholder_id).sum() // frame_num_tokens
                        to_append_num_frames = min(next_turn_num_frames,
                                                   turn_num_frames - 1)  # avoid bias. current as center, two equal left/right side
                        if to_append_num_frames == 0:
                            frame_diff = zero
                        else:
                            to_append_frames = frames[
                                               past_num_frames + turn_num_frames:past_num_frames + turn_num_frames + to_append_num_frames]
                            frame_placeholder = [v_placeholder_id] * frame_num_tokens
                            if use_interval:
                                frame_placeholder = [frame_token_interval_id] + frame_placeholder
                            to_append_input_id = torch.tensor(frame_placeholder * to_append_num_frames,
                            ...
                            idxs = torch.arange(len(frame_placeholder) - 1, len(to_append_input_id),
                                                len(frame_placeholder), device=device)
                            to_append_score = to_append_logit[idxs].softmax(dim=-1)
                            if frame_token_interval_threshold > 0:
                                lower_threshold_mask = to_append_score[:,
                                                       frame_token_interval_id] < frame_token_interval_threshold
...
class LiveLlamaForCausalLM(LlamaForCausalLM, LiveMixin):
    ...
    def forward(
        ...
        if self.dropout and 'task' == task[0]:
        ...
        if labels is not None:
            ...
            if isinstance(single_label, Number):
                ...
                # TODO: multi-label ranked
                exit(2)

            logsoftmax = nn.LogSoftmax(dim=-1).to(cls_logits.device)
            cls_loss = torch.sum(-oh_label * logsoftmax(cls_logits.flatten(0, 1)), dim=-1).mean()

            # for generation
            logits = outputs[0]
            v_mask = input_ids.flatten(0, 1) == self.config.v_placeholder_id
            weight = v_mask * self.config.stream_loss_weight + ~v_mask
            gen_loss = nn.functional.cross_entropy(logits.flatten(0, 1), gen_labels.flatten(),
                                                   reduction='none') * weight
            gen_loss = gen_loss.sum() / (gen_labels >= 0).sum()
...
def build_live_tokenizer_and_update_config(llm_pretrained: str, model_config: LiveConfigMixin) -> AutoTokenizer:
    ...
    model_config.update(
        dict(emb_id=emb_id, v_placeholder_id=v_placeholder_id, frame_token_interval_id=frame_token_interval_id,
             eos_token_id=tokenizer.eos_token_id))
    tokenizer.chat_template = chat_template(tokenizer, get_stream_placeholder_jinja2(model_config))
    tokenizer.get_learn_ranges = partial(get_learn_ranges, chat_template_offsets=chat_template_offsets(tokenizer),
...
def build_live(*, is_training: bool, config_class: type, model_class: type, llm_pretrained: str = None,
               finetune_modules: list[str] = None, lora_modules: str = None, lora_r: int = None, lora_alpha: int = None,
               set_vision_inside: bool = False,
               resume_from_checkpoint: str = '', attn_implementation: str = 'flash_attention_2',
               torch_dtype: str | torch.dtype = 'auto', **kwargs):
    config_cls = config_class.from_pretrained(llm_pretrained, **kwargs)
    config_cls.dropout = kwargs.get('head_dropout', 0.0)
    model = model_class.from_pretrained(llm_pretrained, config=config_cls, torch_dtype=torch_dtype,
...
class StreamMixIn(torch.utils.data.Dataset):
    ...
    def max_frames_clip(self, conversation: list[dict], load_ranges: dict[str, range], max_num_frames: int):
        ...
        for i, message in enumerate(conversation):
            if message['role'] == 'stream':
                ...
                if message['num_frames'] > max_num_frames:
                    sample_indices = np.linspace(0, num_frames - 1, max_num_frames, dtype=int)
                    message['num_frames'] = max_num_frames
                    load_ranges = {path: [ranger.start + int(idx) for idx in sample_indices] for path, ranger in
                                   load_ranges.items()}
        return conversation, load_ranges
    ...
    def __getitem__(self, *, conversation: list[dict], load_ranges: dict[str, range] | torch.Tensor = None,
        ...
        #     print(i, task, repr(text[learn_ranges[i].start: learn_ranges[i].stop]))
...
class COINBenchmark(COIN, StreamMixIn):
    ...
    def __getitem__(self, index):
        ...
        conversation = anno['conversation'] if self.is_training else anno['conversation'][
                                                                     :-1]  # if not training, do not include the assistant message
        return *super().__getitem__(conversation=conversation, load_ranges=anno['load_ranges'], task=task, label=label,
                                    add_generation_prompt=not self.is_training), index, self.evaluation_kwargs
        ...
        for anno in self._annos:
            ...
            for i in range(len(steps)):
                ...
                conversation = [
                    ...
                    {"role": "embed", "content": ''},
                    {"role": "assistant", "content": response + task_response, 'learn': True}
                ]
                self.annos.append({
    ...
    def __init__(self, *, split: str, frame_fps: int, is_training: bool, **kwargs):
        ...
        for anno in self._annos:
            ...
            for i in range(len(steps) - 1):
                ...
                self.labels.append([num_label, steps[i + 1]['text'].lower()])
                start_time = ceil_time_by_fps(steps[i]['start'], frame_fps, min_time=0, max_time=duration)
                end_time = ceil_time_by_fps(steps[i]['end'], frame_fps, min_time=0, max_time=duration)
                start_frame = int(start_time * frame_fps)
                end_frame = int(end_time * frame_fps) + 1
...
class COINTask(COINBenchmark):
    ...
    def __init__(self, *, split: str, frame_fps: int, is_training: bool, **kwargs):
        ...
        for anno in self._annos:
            ...
            self.labels.append([num_label, anno['task'].lower()])
...
####################################################### trainer ######################################################
import time
from transformers.trainer import EvalLoopOutput, DataLoader, deepspeed_init, has_length, EvalLoopContainer, \
    find_batch_size
...
class TrainerWithGenToEval(Trainer):
    ...
    def prediction_step(
        ...
        with torch.no_grad(), self.compute_loss_context_manager():
            ...
            output_ids = getattr(model, evaluator)(**inputs, **evaluation_kwargs,
